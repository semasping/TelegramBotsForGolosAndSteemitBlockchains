    ...
    private function checkPosts($operation)
    {
        ...
        if ($action == 'comment') {
            if ($data['parent_author'] == '' && !str_contains($data['body'],
                ...
                dispatch((new BchProcessNewPostCreated($data))->onQueue(getenv('BCH_API') . 'process_post_created'));
                //dispatch((new GolosProcessNewPost($data))->onQueue('GolosProcessNewPosts'));
                $rt         = $data;
                $golosblock = GolosBlock::firstOrCreate([
                    'id'               => $this->block_id,
                    'raw_transactions' => serialize($rt),
                    'status'           => 'done'
                ]);
                echo 'sent to GolosProcessCreatePost';
